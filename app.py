...
# # ── Make.com webhook for the AI Travel Bot ──
...
# # /* ── AI BOT SECTION ─────────────────────────────── */
# .bot-section{padding:40px 40px 0;}
# .bot-section-head{margin-bottom:28px;}
# .bot-section-head h2{font-family:var(--ff-head);font-size:30px;font-weight:700;color:var(--ink);letter-spacing:-0.6px;margin-bottom:6px;}
# .bot-section-head p{font-size:14.5px;color:var(--ink-soft);line-height:1.6;}

# .bot-card{background:var(--white);border:1px solid var(--border);border-radius:20px;box-shadow:var(--shadow);overflow:hidden;}
# .bot-topbar{background:var(--ink);padding:18px 28px;display:flex;align-items:center;justify-content:space-between;}
# .bot-topbar-left{display:flex;align-items:center;gap:14px;}
# .bot-avatar-wrap{width:44px;height:44px;background:var(--accent);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;}
# .bot-name{font-family:var(--ff-head);font-size:17px;font-weight:600;color:white;line-height:1.2;}
# .bot-status-pill{font-size:11px;color:rgba(255,255,255,0.55);display:flex;align-items:center;gap:5px;}
# .bot-status-pill::before{content:"";width:6px;height:6px;background:#4ade80;border-radius:50%;display:inline-block;animation:blink 2s infinite;}

# .bot-messages{height:440px;overflow-y:auto;padding:24px 28px;display:flex;flex-direction:column;gap:16px;background:var(--cream);}
# .bot-messages::-webkit-scrollbar{width:4px;}
# .bot-messages::-webkit-scrollbar-thumb{background:var(--border);border-radius:99px;}

# .bot-empty-state{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px;}
# .bot-empty-icon{font-size:52px;margin-bottom:16px;}
# .bot-empty-state h4{font-family:var(--ff-head);font-size:20px;font-weight:600;color:var(--ink);margin-bottom:8px;}
# .bot-empty-state p{font-size:13.5px;color:var(--ink-soft);max-width:340px;line-height:1.6;}

# .bot-bubble-row{display:flex;align-items:flex-end;gap:10px;}
# .bot-bubble-row.user{flex-direction:row-reverse;}
# .bot-icon-circle{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;}
# .bot-icon-circle.ai{background:var(--accent);color:white;}
# .bot-icon-circle.human{background:var(--ink);color:white;}
# .bot-bubble{max-width:72%;padding:13px 17px;border-radius:18px;font-size:14px;line-height:1.65;color:var(--ink);}
# .bot-bubble.ai{background:var(--white);border:1px solid var(--border);border-bottom-left-radius:4px;box-shadow:0 1px 6px rgba(0,0,0,0.06);}
# .bot-bubble.user{background:var(--ink);color:white;border-bottom-right-radius:4px;}
# .bot-bubble-time{font-size:10.5px;color:var(--ink-soft);margin-top:4px;text-align:right;}
# .bot-bubble-row.user .bot-bubble-time{text-align:left;}

# .bot-chips{display:flex;flex-wrap:wrap;gap:8px;padding:14px 28px 0;}
# .bot-chip-label{font-size:11.5px;font-weight:600;color:var(--ink-soft);letter-spacing:.5px;text-transform:uppercase;margin-bottom:4px;padding:0 28px;}

# .bot-typing{display:flex;align-items:center;gap:5px;padding:10px 14px;}
# .bot-typing span{width:7px;height:7px;background:var(--ink-soft);border-radius:50%;animation:typing 1.2s ease-in-out infinite;}
...
# # Uses st.components to inject into parent
# # ─────────────────────────────────────────
...
# </style>
# <script>
# // Inject FAB + chat window into the PARENT Streamlit page
# (function() {{
...
#       background:#1A1714; color:white; font-size:26px;
#       display:flex; align-items:center; justify-content:center;
#       cursor:pointer;
#       box-shadow:0 6px 32px rgba(26,23,20,0.38);
...
#     #voy-chatwin {{
#       position:fixed; bottom:106px; right:28px; z-index:99998;
...
#     .vt-x {{ background:rgba(255,255,255,.1); border:none; color:white; width:30px; height:30px; border-radius:50%; cursor:pointer; font-size:15px; display:flex; align-items:center; justify-content:center; transition:background .2s; }}
...
#     .vc:hover {{ background:#C4622D; color:white; border-color:#C4622D; }}

#     #voy-msgs {{ flex:1; overflow-y:auto; padding:14px 12px; display:flex; flex-direction:column; gap:11px; scroll-behavior:smooth; }}
...
#     .vb {{ max-width:78%; padding:9px 13px; border-radius:15px; font-size:13px; line-height:1.6; word-wrap:break-word; }}
...
#     #voy-bar {{ padding:10px 12px; background:white; border-top:1px solid #E8E4DE; flex-shrink:0; display:flex; gap:8px; align-items:center; }}
#     #voy-in {{
...
#     #voy-go {{
#       width:36px; height:36px; border-radius:50%; background:#1A1714;
...
#     #voy-clr {{ background:none; border:none; color:#b0ada9; font-size:16px; cursor:pointer; padding:0 2px; transition:color .15s; flex-shrink:0; }}
...
#   parent.voyToggle = function() {{
#     isOpen = !isOpen;
#     pd.getElementById('voy-chatwin').classList.toggle('open', isOpen);
#     pd.getElementById('voy-fab').classList.toggle('open', isOpen);
#     if (isOpen) {{ unread=0; badge(); setTimeout(()=>pd.getElementById('voy-in').focus(),280); }}
#   }};
...
#     fetch(WEBHOOK, {{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{message:text}})}})
#       .then(r=>r.text())
#       .then(rep=>{{
#         hideDots();
...
#       }})
#       .catch(()=>{{hideDots();msgs.push({{r:'ai',t:'⚠️ Connection error. Please try again.',ts:ts()}});render();}})
...
#     box.innerHTML='';
#     if(!msgs.length){{
#       box.innerHTML=`<div class="ve"><div class="ve-i">🌍</div><h4>Your travel assistant is ready</h4><p>Ask anything — destinations, visa, packing,<br>food, safety, hidden gems.</p></div>`;
#       return;
#     }}
#     msgs.forEach(m=>{{
#       const row=pd.createElement('div'); row.className='vr'+(m.r==='u'?' u':'');
#       const ic=m.r==='u'?'<div class="vi hu">👤</div>':'<div class="vi ai">✈️</div>';
#       const bc=m.r==='u'?'u':'ai';
#       const lb=m.r==='u'?'You':'Voyagr AI';
#       row.innerHTML=`${{ic}}<div><div class="vb ${{bc}}">${{m.t}}</div><div class="vbt">${{lb}} · ${{m.ts}}</div></div>`;
#       if(typEl) box.insertBefore(row,typEl); else box.appendChild(row);
#     }});
#     scroll();
#   }}
...
#     typing=true;
#     const box=pd.getElementById('voy-msgs');
#     typEl=pd.createElement('div'); typEl.className='vr';
...
#   function scroll() {{ const b=pd.getElementById('voy-msgs'); setTimeout(()=>b.scrollTop=b.scrollHeight,60); }}
...
# ─────────────────────────────────────────
# FLOATING CHAT BUBBLE  — fully responsive
# ─────────────────────────────────────────
...
(function(){{
  ...
  style.textContent = `
    ...
    /* ── Safe area close button for notch phones ── */
